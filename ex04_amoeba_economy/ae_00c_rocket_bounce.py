...
Preliminaries, step C:
Now the smooth bouncing back. Move the ball with the arrow keys, hit the wall.
"""
import numpy as np
...
class Amoeb(object):
    ...
    def wall_repulsion(self):
        ...
        if distances['left'] < dlim:
            self.f[0] += strength*bounce_force(distances['left'],dlim)
        if distances['right'] < dlim:
            self.f[0] -= strength*bounce_force(distances['right'],dlim)
        if distances['top'] < dlim:
            self.f[1] -= strength*bounce_force(distances['top'],dlim)
        if distances['bottom'] < dlim:
            self.f[1] += strength*bounce_force(distances['bottom'],dlim)
